//Ejecutar el programa en node como "ts-node StringAdd.ts '<Cadena a sumar>'"
//Se obtiene la cadena de los argumentos del programa
const argv:Array<String> = process.argv
const cadena:String = argv[2]
...
try {
  ...
  const suma:number = cadena.split('').map((value)=>value.charCodeAt(0)).reduce((acc,current)=>acc+current)
